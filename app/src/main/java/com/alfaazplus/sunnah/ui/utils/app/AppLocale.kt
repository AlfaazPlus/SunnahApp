...
data class AppLocale(
    ...
    fun fallbackLanguageCodes(default: String = "en"): Sequence<String> = sequenceOf(baseLocale.toLanguageTag(), baseLocale.language, default)
}
...
fun numeralSystemsForLanguage(language: String): List<Pair<NumeralSystem, String>> = when (language.lowercase(Locale.ROOT)) {
    "ar" -> listOf(
...
fun readAppLocale(context: Context): AppLocale {
    ...
    return AppLocale(
        ...
        numeralSystem = coerceNumeralForLanguage(baseLocale.language, NumeralSystem.LATN),
    )
}
...
fun appFallbackLanguageCodes(default: String = "en"): Sequence<String> = appLocale().fallbackLanguageCodes(default)
